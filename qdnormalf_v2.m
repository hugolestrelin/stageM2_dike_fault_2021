...
function db=evoldyke(t,bmax,nbd,tdike) % t = time dans le loop ; tdike = time du dike ; bmax = rate d'ouverture maximal ; nbd = nombre de jours (en secondes) sur lesquels s'effectue l'ouverture
sigd=4*nbd;
db=exp(-1/2*((t-tdike)/sigd)^2)/(sig*sqrt(2*pi));
end