...
param.zsill=-2000; %--profondeur du sill, 0 correspond à la surface de l'océan
...
param.tfinal=25*year; %--temps de modélisation en année
...
dike_deficit=zeros(size(par.dsigOP));
...
while t<par.tfinal
    ...
    if t>par.flaguprev
        if max(sigOP)>par.OPdike && param.dyke_act==0 % calculer deux dsigm : dike (pa/m) et decompression sill (pa instantané) car pas m^ evol (donc adimensionalisation)
            ...
            b=dike_deficit(find(sigOP==max(sigOP)))*(param.rho*param.g*param.z)/(abs(SIG_dyke_NR.xx));
...
%             end
        ...
        if param.dyke_act==1
            ...
            sigOP=sigOP+(par.k*par.b+dbi*par.dsigOP_dike-dpi*par.dsigOP_sbh+par.dsigOPfault*exp(phi))*dt; %/ simple addition constante des résultats du champs des contraintes non ?
            %sigOP=sigOP+(par.k*par.b-dpi*dike_deficit_prev/par.POP-dpi*par.dsigOP_sbh+0*par.dsigOPfault*exp(phi))*dt; %/ simple addition constante des résultats du champs des contraintes non ?
            %disp(sigOP)
            %disp(dbi)
            %disp(dpi)
            sigOP(sigOP>0)=0;
            dike_deficit=dike_deficit+par.k*par.b*dt+dbi*par.dsigOP_dike;
        else
