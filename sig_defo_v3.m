...
for k=1:rr(2)
    ...
    elseif sind(thetaf)==0
        xfp=xsidown(1):(Lf*cosd(thetaf))/9:xsiup(1); %!!! SEULEMENT SI FAILLE À DROITE SILL, AJOUTER CONDITION SI NON !!!
    ...
    %faire un dike :
    xOP=-L/2-3*pas:pas:L/2+3*pas;%-L/2-2*pas:pas:L/2+2*pas; % demi-sill : 0:pas:L/2+3*pas; %% SI BESOIN EFFECTUER CHANGEMENT TAILLE VECTEUR D'ÉVALUATION DES SIG AU DESSUS DU SILL
    sxop=size(xOP);
