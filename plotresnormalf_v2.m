...
cumulsigop=reshape(cumulsigop,[],isave);
subplot(3,2,5,'align');
ssigOP=size(sigOP);
...
colorbar
hold on
plot(catdike.where,catdike.t,'p','MarkerEdgeColor','black','MarkerFaceColor','black','MarkerSize',15)
...
subplot(3,2,6,'align');
r=plot(1:isave,cumulsigop(find(sigOP==max(sigOP)),:),'color','k');%find(sigOP==max(sigOP)) ; find(xOP==catdike.where(1))
set(r,'XData',round((0:par.tfinal/(isave-1):par.tfinal)))
